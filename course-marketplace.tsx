...
export default function CourseMarketplace() {
  ...
  useEffect(() => {
    async function fetchCourses() {
      ...
      const { data, error } = await supabase
        .from("courses")
        .select("id,title,price,duration,difficulty,environment,photo_url,location,schools(name),status,course_type")
        .eq('status', 'Published')
      if (error) {
  ...
  useEffect(() => {
    async function fetchCourseTypeOptions() {
      ...
      if (!error && data) {
        ...
        data.forEach((course) => {
          if (course.course_type && typeof course.course_type === 'string' && course.course_type.trim() !== "") {
            allTypes.push(course.course_type)
          }
        })
        const uniqueTypes = Array.from(new Set(allTypes)).sort()
        setCourseTypeOptions(uniqueTypes)
      }
    }
  ...
  const filteredCourses = useMemo(() => {
    let filtered = courses.filter((course) => {
      ...
      // Course type filter
      if (filters.courseTypes.length > 0 && !filters.courseTypes.includes(course.course_type)) {
        return false
      }
      // Difficulty filter
    ...
    <div className="text-zinc-100 pt-20 md:pt-0">
      ...
      <div className="container mx-auto px-4 py-8">
        <div className="flex items-start justify-between">
          <div>
            <h1 className="text-4xl font-bold text-white mb-2 flex items-baseline gap-3">
              <span>
              ...
              {hasActiveFilters && (
                <span className="text-zinc-400 text-base font-normal">
                  <span className="font-bold">of {courses.length}</span>
                </span>
              )}
            </h1>
          </div>
          {/* Desktop search bar (hidden on mobile) */}
          <div className="hidden md:flex flex-col items-end w-full max-w-md">
            <form
