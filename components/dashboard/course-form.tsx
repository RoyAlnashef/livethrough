...
import { MapPin, DollarSign, Clock, Star, ImageIcon, X, Plus } from "lucide-react"
...
export function CourseForm({ mode, initialValues, onSubmit, isSubmitting }: CourseFormProps) {
  ...
  useEffect(() => {
    const fetchCourseTypes = async () => {
      // Fetch all course types from the 'courses' table.
      const { data, error } = await supabase.from("courses").select("course_type")
      if (error) {
        console.error("Error fetching course types:", error.message)
      } else if (data) {
        const fetchedCourseTypes = [
          ...new Set(data.map((item) => item.course_type).filter((type): type is string => !!type)),
        ]
        ...
        <div className="col-span-2 space-y-6">
          ...
          <Card className="bg-zinc-900 border-zinc-800">
            <CardHeader>
              ...
              <CardDescription className="text-zinc-400">Essential course details and description</CardDescription>
            </CardHeader>
          ...
          <Card className="bg-zinc-900 border-zinc-800">
            <CardHeader>
              ...
              <CardDescription className="text-zinc-400">
                Pricing, duration, difficulty, and skills settings
              </CardDescription>
            </CardHeader>
            <CardContent className="space-y-4">
              ...
              <div className="grid grid-cols-2 gap-4">
                ...
                <div>
                  ...
                  <Select
                    ...
                    onValueChange={(value) => setCourse((prev) => ({ ...prev, course_type: value }))}
                  >
                    <SelectTrigger className="bg-zinc-800 border-zinc-700 text-zinc-50">
                    ...
                    <SelectContent className="bg-zinc-800 border-zinc-700">
                      {courseTypes.map((type) => (
                        <SelectItem key={type} value={type} className="text-zinc-50 focus:bg-zinc-700">
                          {type}
                        </SelectItem>
                      ))}
                    </SelectContent>
              ...
              {/* Location & Environment (moved here) */}
              <div>
        ...
        <div className="space-y-6">
          ...
          <Card className="bg-zinc-900 border-zinc-800">
            <CardHeader>
              ...
              <CardDescription className="text-zinc-400">
                Select your school to display your contact info on this course.<br />
                <span className="text-xs text-zinc-500">A school is only required to publish a course. Drafts can be saved without a school.</span>
              </CardDescription>
            </CardHeader>
            <CardContent className="space-y-4">
              <div>
